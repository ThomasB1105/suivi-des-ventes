...
async function buildLeads(cmd) {
  ...
  for (let i = 0; i < aliasFlat.length; i += 2) aliases[String(aliasFlat[i]).toLowerCase()] = aliasFlat[i + 1];
  const resolve = (n) => { const k = String(n || "").trim().toLowerCase(); return (k && aliases[k]) || n; };
...
module.exports = { buildLeads, STAGES, personStats, isPerson, pickNextAssignee, DEFAULT_ALIASES };
