...
const STAGES = ["new", "setting", "booked", "show", "noshow", "won", "lost", "unqualified", "dead", "nrp"];
...
async function buildLeads(cmd) {
  ...
  // ---- Matching email OU téléphone ----
  // Un call (ou un paiement) arrivé avec un AUTRE email que la fiche CRM est
  // rattaché via le numéro de téléphone : phone -> email canonique de la fiche.
  const normPhone = (p) => { const d = String(p || "").replace(/\D/g, ""); return d.length >= 8 ? d.slice(-9) : ""; };
  const phoneOwner = {};
  Object.entries(crmLeads)
    .sort((a, b) => ((b[1].bookedAt ? 1 : 0) - (a[1].bookedAt ? 1 : 0)) || String(a[1].createdAt || "").localeCompare(String(b[1].createdAt || "")))
    .forEach(([em, l]) => { const ph = normPhone(l.phone); if (ph && !phoneOwner[ph]) phoneOwner[ph] = String(em).toLowerCase(); });
  const canon = (email, phone) => {
    const em = String(email || "").toLowerCase();
    if (em && crmLeads[em]) return em;                 // email connu -> lui-même
    const ph = normPhone(phone);
    return (ph && phoneOwner[ph]) || em;               // sinon rattachement par téléphone
  };

  // Ventes encaissées par email (dédup cross-source ±1 jour, même logique que
  // /api/sales) + détail PAR MOIS (commissions/revenu du mois = cash du mois).
  const saleByEmail = {};
  const paidMonthsByEmail = {}; // email -> { "2026-09": 796, ... }
  const lastPaidByEmail = {};   // email -> dernière date de paiement
  {
    ...
    Object.values(ev).forEach((e) => {
      ...
      const em = canon(e.email, e.phone);
      if (!em) return;
      ...
      if (ym) {
        if (!paidMonthsByEmail[em]) paidMonthsByEmail[em] = {};
        paidMonthsByEmail[em][ym] = (paidMonthsByEmail[em][ym] || 0) + amount;
      }
      if (!lastPaidByEmail[em] || String(e.date) > lastPaidByEmail[em]) lastPaidByEmail[em] = String(e.date);
    });
  }
  ...
  Object.values(icCallsById).forEach((c) => {
    const em = canon(c.email, c.phone);
    if (!em) return;
  ...
  // Contacts iClosed rattachés par téléphone quand l'email diffère
  Object.keys(icContacts).forEach((em) => {
    const c = icContacts[em];
    const target = canon(em, c && c.phone);
    if (target !== String(em).toLowerCase() && !icContacts[target]) { icContacts[target] = c; delete icContacts[em]; }
  });
  ...
  return [...emails].map((email) => {
    ...
    if (l.bookedAt) auto = l.showUp === "cancelled" ? "setting" : "booked"; // RDV annulé côté plateforme -> retour setting
    if (call) auto = CALL_TO_STAGE[call.status] || auto;
    if (call && call.hadWon) auto = "won";
    if (paid > 0) auto = "won";
    let stage = l.manualStage && l.stage ? l.stage : auto;
    // Un paiement ENCAISSÉ fait toujours foi : la vente collectée passe le
    // lead en Closé (même si un statut avait été saisi à la main) -> stats,
    // revenu et commission du closer à jour immédiatement.
    if (paid > 0) stage = "won";
    ...
    return {
      email,
      // hasCall : lead avec un call pris (iClosed/Calendly) ou un paiement.
      // Le board CRM n'affiche que ceux-là ; les opt-ins restent visibles dans
      // l'espace setter (« à qualifier »).
      hasCall: !!(call || l.bookedAt || paid > 0),
      name: l.name || email,
      ...
      fathom: l.fathom || undefined,
      // Process setting : lead appelé + groupe WhatsApp créé (cochés par le setter)
      setCalled: l.setCalled || undefined,
      waGroup: l.waGroup || undefined,
      // Statut setting (NRP / cancel / groupe WA créé / non qualifié), partagé
      // avec la vue closing. Compat : waGroup coché => "wa".
      setStatus: l.setStatus || (l.waGroup ? "wa" : undefined),
      // Suivi de récupération (onglet Saphia) : contacté / follow-up / re-booké…
      outreach: l.outreach || undefined,
      outreachAt: l.outreachAt || undefined,
      showUp: l.showUp || autoShowUp || undefined,
      callResult: l.callResult || autoResult || undefined,
      followUp: l.followUp || autoFollow || undefined,
      followUpAt: l.followUpAt || undefined, // date de relance choisie par le closer
      answers: Object.keys(ansMerged).length ? ansMerged : undefined,
      amount: paid || undefined,
      paidMonths: paidMonthsByEmail[email] || undefined, // cash encaissé par mois
      paidAt: lastPaidByEmail[email] || undefined,
    ...
    // Données de test du branchement webhook exclues. Les opt-ins VSL sans
    // call sont GARDÉS (hasCall=false) : le front les écarte du board mais les
    // montre dans l'espace setter (« à qualifier »).
...
function personStats(leads, name, role, rate) {
  ...
  // Stats sur les CALLS uniquement (les opt-ins à qualifier ne comptent pas).
  const mine = leads.filter((l) => isPerson(l[field], name) && l.hasCall !== false);
